...
for (var i = 0; i < finances.length; i++) {
    ...
    // add to to increases array and if it's a negative, add to separate array called decreases
    if (diff >= 0) {
        increases.push(diff);
    } else if (diff < 0) {
...
//sort the increases array which put the biggest increase at the start of the array
//capture that first element as the biggest increase
var greatestIncrease = increases.sort()[0];


// The greatest decrease in profits (date and amount) over the entire period.
//sort the array of negative changes i.e. decreases
//doing this by default will put the highest number first.
//however, this is not biggest decrease so we have to reverse order it and grab the first element as greatest decrease.
